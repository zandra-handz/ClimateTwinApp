...
const { friends, handleSendFriendRequest, friendRequestMutation, searchUsersMutation } = useFriends();
...
const handleToggleDoubleChecker = () => {
 ...
 useEffect(() => {
  if (friendRequestMutation.isSuccess) {
    showAppMessage(true, null, `Friend request has been sent to ${username}!`);
    router.back();
  }
 ...
 useEffect(() => {
  if (friendRequestMutation.isError) {
    showAppMessage(true, null, `Oops! Did you already send a friend request to ${username}?`);
    router.back();
  }
...
const handleAddFriend = () => {
 ...
 useEffect(() => {
     if (friends) {
         const checkForFriendship = friends?.find((friend) => (friend.friend === Number(id)));
